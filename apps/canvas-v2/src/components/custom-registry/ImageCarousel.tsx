...
export const ImageCarouselComponent: React.FC<{
  ...
  // Responsive image width: smaller on mobile/tablet
  ...
  // Need at least 2 copies for seamless loop, use 4 for safety
  const duplicatedImages = [...images, ...images, ...images, ...images];

  // Unique ID for scoped keyframes (avoid conflicts with multiple carousels)
  const uniqueId = element.id || "carousel";

  // Calculate total width of one set of images for precise animation
    ...
    >
      ...
      <div className="relative w-full" style={{ height: effectiveHeight }}>
        {images.length > 0 ? (
          ...
          >
            {duplicatedImages.map((img: any, idx: number) => (
              <div
                key={idx}
...
export const ImageCarouselDef: CustomComponentDef = {
  name: "Image Carousel",
