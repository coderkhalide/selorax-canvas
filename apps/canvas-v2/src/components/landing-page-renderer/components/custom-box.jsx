...
const DynamicIcon = ({ name, size = 24, color }) => {
...
const BoxesComponent = ({
  ...
  const renderItem = (item, index, customStyle) => {
    const baseCardStyle = {
        ...
        <div className="flex flex-col gap-4 h-full">
          <div className="flex items-center justify-between">
            {item.icon && (
              ...
              >
                <DynamicIcon name={item.icon} size={iSize} color="#ffffff" />
              </div>
            )}
            <div
  ...
  if (layout === "grid-3") finalCols = 3;
  if (layout === "grid-4") finalCols = 4;
